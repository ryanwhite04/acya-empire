...
class acyaempire_widget_testimonial extends WP_Widget {
	function __construct() {
		$widget_ops = array( 'classname' => 'widget_testimonial', 'description' => __( 'Display Testimonial on FrontPage', 'acya-empire' ) );
		$control_ops = array( 'width' => 200, 'height' =>250 ); 
		parent::__construct( false, $name = __( 'TF: FP Testimonial', 'acya-empire' ), $widget_ops, $control_ops);
	}
	function form($instance) {
		...
		<label for="<?php echo $this->get_field_id('image'); ?>">
			<?php _e( 'Upload Testimonial Background Image', 'acya-empire' ); ?>
		</label>
  ...
  <input type="button" class="button  custom_media_button"name="<?php echo $this->get_field_name('image'); ?>" id="custom_testimonial" value="<?php _e('Upload Image','acya-empire');?>" onclick="mediaupload.uploader( '<?php echo $this->get_field_id( 'image' ); ?>' ); return false;"/>
		</p>
		<p>
		...
		<p>
			...
			<?php _e( 'Number of Testimonial:', 'acya-empire' ); ?>
			</label>
		...
		<div class="testimonial_bg" <?php if(!empty($image)){ ?>style="background-image:url('<?php echo esc_url($image); ?>');" <?php } ?> >
			<div class="container clearfix">
				<div class="testimonials">
					<div class="quote-wrapper acya-animation fadeInRight">
					<?php
			 			$i=1;
